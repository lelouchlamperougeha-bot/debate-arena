  ...
  if(stage==="setup") return(
    <div style={{height:"100vh",background:"#0a0a0f",color:"#e8e4dc",fontFamily:"Georgia,serif",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        ...
        {/* LEFT — Topic */}
        <div style={{flex:1,padding:"28px 36px",borderRight:BDR,display:"flex",flexDirection:"column",gap:0}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:16}}>
            {topics.map(t=>(
              <button key={t.label} className="hov" onClick={()=>{setTopic(t.label);setCustom("");}}
                style={{background:topic===t.label&&!custom?"#1e1c2e":"#0f0f16",border:topic===t.label&&!custom?"1.5px solid #534AB7":BDR,borderRadius:10,padding:"14px 16px",fontSize:15,fontFamily:"sans-serif",color:topic===t.label&&!custom?"#b8b0f0":"#c0bdb8",textAlign:"left",lineHeight:1.5}}>
          ...
          <div style={{marginTop:"auto",paddingTop:16,display:"flex",gap:14,flexWrap:"wrap",alignItems:"center"}}>
        ...
        {/* MIDDLE — Side + Style */}
        <div style={{flex:1,padding:"28px 36px",borderRight:BDR,display:"flex",flexDirection:"column",gap:28}}>
          <div>
            <div style={{fontSize:12,fontWeight:700,letterSpacing:".14em",textTransform:"uppercase",color:G,fontFamily:"sans-serif",marginBottom:16}}>Your Side</div>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              {[["for","👍","I'm FOR it","You argue in favor — Claude argues against"],
                ...
                <div key={v} className="hov" onClick={()=>setSide(v)}
                  style={{background:side===v?"#1e1c2e":"#0f0f16",border:side===v?"1.5px solid #534AB7":BDR,borderRadius:12,padding:"16px 18px",cursor:"pointer"}}>
                  <div style={{display:"flex",alignItems:"center",gap:12}}>
                    <span style={{fontSize:26}}>{ic}</span>
                    <div>
                      <div style={{fontSize:16,fontWeight:700,color:"#e8e4dc",fontFamily:"sans-serif"}}>{ti}</div>
                      <div style={{fontSize:13,color:"#9a9690",fontFamily:"sans-serif",marginTop:3}}>{de}</div>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:9}}>
              {Object.entries(INTENSITY).map(([k,{label}])=>(
                <button key={k} className="hov" onClick={()=>setIntensity(k)}
                  style={{background:intensity===k?"#1a1208":"#0f0f16",border:intensity===k?`1.5px solid ${G}`:BDR,borderRadius:9,padding:"13px 16px",fontSize:15,fontFamily:"sans-serif",color:intensity===k?G:"#c0bdb8",textAlign:"left",cursor:"pointer"}}>
                  {label}
                </button>
              ))}
            </div>
        ...
        <div style={{flex:1,padding:"28px 36px",display:"flex",flexDirection:"column"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:18}}>
              {TRAITS.map(t=>(
                <button key={t.id} className="hov" onClick={()=>toggleTrait(t.id)}
                  style={{background:traits.includes(t.id)?"#1a1208":"#0f0f16",border:traits.includes(t.id)?`1.5px solid ${G}`:BDR,borderRadius:9,padding:"12px 14px",cursor:"pointer",textAlign:"left"}}>
          ...
          {/* Scoring guide */}
          <div style={{background:"#0f0f16",border:BDR,borderRadius:12,padding:"18px 20px",marginTop:"auto",marginBottom:20}}>
            <div style={{fontSize:12,fontWeight:700,color:G,fontFamily:"sans-serif",marginBottom:14,textTransform:"uppercase",letterSpacing:".1em"}}>How Scoring Works</div>
            {[["🟢","Strong (8-10)","#4ade80","You gain the most points"],
              ...
              <div key={label} style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                <span style={{fontSize:15,flexShrink:0}}>{dot}</span>
                <div>
                  <span style={{fontSize:14,color:col,fontFamily:"sans-serif",fontWeight:600}}>{label} </span>
                  <span style={{fontSize:14,color:"#c0bdb8",fontFamily:"sans-serif"}}>{explain}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <button disabled={!act||!side} className={act&&side?"bhov":""} onClick={startDebate}
            style={{width:"100%",padding:"18px",background:act&&side?G:"#1a1a24",color:act&&side?"#0a0a0f":"#444",border:"none",borderRadius:12,fontSize:18,fontWeight:700,fontFamily:"sans-serif",letterSpacing:".05em",cursor:act&&side?"pointer":"not-allowed",transition:"all .2s"}}>
